...
pub fn read_mnist_images(imagefile: &str, labelfile: &str) -> io::Result<Vec<MnistImage>> { //-> io::Result<&'static [u8]> {
    ...
    let num_image_items = bytes_to_u32(&u32buf0);
    label_buf.read(&mut u32buf1);
    // i don't know how to get int out of 4 bytes using a rust library.
    let num_label_items = bytes_to_u32(&u32buf1);
    ...
    image_buf.read(&mut u32buf0[..]);
    let row_dimension = bytes_to_u32(&u32buf0);
    if row_dimension != 28 {
        return Err(io::Error::new(io::ErrorKind::Other,
                    format!("the row dimension {} is not 28.", row_dimension)))
    }
    image_buf.read(&mut u32buf1[..]);
    let col_dimension = bytes_to_u32(&u32buf1);
    if col_dimension != 28 {
        return Err(io::Error::new(io::ErrorKind::Other,
                    format!("the col dimension {} is not 28.", col_dimension)))
    }
    //let mut images: Vec<MnistImage> = Vec::with_capacity(num_image_items as usize);
    let mut images = vec![MnistImage { data: [0u8; IMAGE_SIZE], label: 0u8 }; num_image_items as usize];
    for i in 0..num_image_items {
        let one_image = &mut images[i as usize]; // IMAGE_SIZE * i..IMAGE_SIZE * (i+1)];
        image_buf.read(&mut one_image.data);
        let mut one_label = [0u8];
        label_buf.read(&mut one_label);
